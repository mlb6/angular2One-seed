...
gulp.task("protractor:only", ["webdriver-update"], function (done) {
  ...
  gulp.src(pathCfg.dest.buildE2E)
    .pipe($.protractor.protractor({configFile: pathCfg.protractorConfig}))
...
gulp.task("protractor-sauce", function (done) {
  gulp.src(pathCfg.dest.buildE2E)
    .pipe($.protractor.protractor({configFile: pathCfg.protractorSauceConfig}))
